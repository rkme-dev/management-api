...
final class UpdateCollectionController extends AbstractAPIController
{
    ...
    {
        OrderItem::where('orderable_type', 'App\Models\Collection')
            ->where('orderable_id', $collection->getAttribute('id'))
            ->delete();
    ...
    private function createSalesDrPayments(
        ...
        SalesDrPayment::where('collection_id', $collection->getAttribute('id'))->delete();
        ...
        foreach ($salesDrs as $salesDr) {
            SalesDrPayment::create([
                ...
                'sales_dr_id' => Arr::get($salesDr, 'sales_dr_id'),
            ]);
        }
    }
    ...
    private function createCollectionPayments(
        ...
        CollectionPayment::where('collection_id', $collection->getAttribute('id'))
            ->delete();
