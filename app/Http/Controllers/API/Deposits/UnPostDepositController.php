...
final class UnPostDepositController extends AbstractAPIController
{
    public function __invoke(int $id): JsonResource
    {
        ...
        $deposit->setAttribute('status', SaleOrderStatusesEnum::FOR_REVIEW->value);
        $deposit->save();

        // @TODO put in a service and run each check in a job
        /** @var CheckPayment $check */
        foreach ($deposit->checks as $check) {
            $check->setAttribute('status', CheckStatusEnums::UNCOLLECTED->value);
            $check->save();
        }
